...
export const LoggedInDashboard: React.FC<LoggedInDashboardProps> = ({
  ...
  const email =
    ...
    claimText(claims['upn']) ||
    'No email';
  const birthdate = userInfo?.birthdate || claimText(claims['birthdate']);
  const sex = userInfo?.sexAtBirth || claimText(claims['sex_at_birth']);
  const profilePicUrl =
    userInfo?.picture ||
    claimText(claims['citizen_profile_picture']) ||
    claimText(idClaims['citizen_profile_picture']);
