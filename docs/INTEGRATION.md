...
2. Your app asks your backend for a PAR `request_uri`, sending the
   `codeChallenge`, the `state`, the `environment` and the `redirectUri` (and
   optionally `nonce` and `scope`). The `codeVerifier` stays in the app. The full request
   and response bodies are in
...
3. Your app calls the SDK's `authenticate` with that `request_uri` and that same `state`.
...
| --- | --- |
| [Android](https://github.com/ditkrg/krdpass-auth-sdk-android) | Maven Central `krd.pass:krdpass-auth:1.4.0` |
| [iOS](https://github.com/ditkrg/krdpass-auth-sdk-ios) | SwiftPM or CocoaPods, git tag `v1.4.0` |
| [Flutter](https://github.com/ditkrg/krdpass-auth-sdk-flutter) | pubspec git dependency, `ref: v1.4.0` |
| [React Native](https://github.com/ditkrg/krdpass-auth-sdk-react-native) | npm git dependency, `#v1.4.0` |
