...
"v1" in this document refers only to the CAA bootstrap envelope, never to an SDK
release. See [CAA Bootstrap Contract](#caa-bootstrap-contract).
...
- [Redirect Validation Rules](#redirect-validation-rules)
- [ID Token Audience Rules](#id-token-audience-rules)
- [CAA Bootstrap Contract](#caa-bootstrap-contract)
- [BFF Token Exchange Contract](#bff-token-exchange-contract)
- [Native Caller Trust](#native-caller-trust)
- [Protocol Parity](#protocol-parity)
...
- `getUserInfo(accessToken)`: retrieves user profile claims. The typed fields
  ...
  (`sub` is a third case: stored as the internal primary key, never displayed.)
...
**Known asymmetry with the reference BFF.** `server/cas.js` checks `aud` by
...
`redirect_uri` as required by OAuth; it is a separate contract, documented in
[`cas-oauth-endpoints.swagger.json`](../../shared/contracts/cas-oauth-endpoints.swagger.json).
