...
Required:
  --app <name>            One of: server, flutter, android, ios, react-native
                          (android-native / ios-native still supported as aliases)

Options:
  --platform <name>       Required for flutter/react-native: android | ios
  --device <id-or-name>   Device target for flutter/ios/react-native ios
                           (if omitted for flutter, script auto-selects matching platform device)
                           (if omitted for ios-native/react-native ios, script prefers connected iPhone)
  ...
  --no-dev-server         For react-native iOS: skip starting Expo dev server after install
...
stop_project_expo_on_port() {
  ...
  while IFS= read -r pid; do
    ...
    if [[ "$proc_cmd" == *"$project_dir"* && "$proc_cmd" == *"expo "* ]]; then
      info "stopping existing Expo process on :${port} (pid=$pid)"
...
run_react_native() {
  ...
  if [[ "$PLATFORM" == "ios" ]]; then
    stop_project_expo_on_port "$dir" "8081"
    sleep 1
  fi
  ...
  if [[ "$PLATFORM" == "ios" ]]; then
    ...
    if is_metro_running "8081"; then
      ...
      env -u CI npx expo start --dev-client --tunnel --scheme krdpass --port 8081
    fi
  fi
}
