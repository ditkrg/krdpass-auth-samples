...
Options:
  ...
  --patch-tracked          Rewrite tracked sample sources (package ids, bundle ids,
                           entitlements, the iOS scheme). Off by default.
...
# Append a value to a comma-separated list unless it is already in it. The entries are
# not trimmed here because the server trims and drops blanks when it reads the list.
csv_append_unique() {
  local csv="${1:-}"
  local value="${2:-}"
...
# The caller's environment wins over the secrets file for these documented
# overrides (docs/BUILDING.md), so capture them before sourcing sets them.
CALLER_DEMO_EXTRAS="${DEMO_EXTRAS-}"
CALLER_DEMO_EXTRAS_SET="${DEMO_EXTRAS+set}"
CALLER_DEMO_UNAUTHENTICATED_TOKEN_ROUTES="${DEMO_UNAUTHENTICATED_TOKEN_ROUTES-}"
CALLER_DEMO_UNAUTHENTICATED_TOKEN_ROUTES_SET="${DEMO_UNAUTHENTICATED_TOKEN_ROUTES+set}"
...
if [[ -n "$CALLER_DEMO_EXTRAS_SET" ]]; then
  DEMO_EXTRAS="$CALLER_DEMO_EXTRAS"
fi
if [[ -n "$CALLER_DEMO_UNAUTHENTICATED_TOKEN_ROUTES_SET" ]]; then
  DEMO_UNAUTHENTICATED_TOKEN_ROUTES="$CALLER_DEMO_UNAUTHENTICATED_TOKEN_ROUTES"
fi
...
# .kt/.java package trees, pbxproj, entitlements, app.json, the iOS scheme),
...
# Opt-in on purpose: these two routes lend CLIENT_SECRET to any caller that can
# reach the port. Typing the flag is the moment the warning in server/README.md
# lands, so this does not default it on for you.
DEMO_UNAUTHENTICATED_TOKEN_ROUTES="${DEMO_UNAUTHENTICATED_TOKEN_ROUTES:-false}"
...
mkdir -p "$ROOT_DIR/react-native-bare"
if [[ "$TARGET_ANDROID" == "true" ]]; then
...
DEMO_UNAUTHENTICATED_TOKEN_ROUTES=$DEMO_UNAUTHENTICATED_TOKEN_ROUTES
...
KRD_ENVIRONMENT=$KRD_ENVIRONMENT
EOF_FLUTTER_ENV
...
EXPO_PUBLIC_KRD_ENVIRONMENT=$KRD_ENVIRONMENT
EOF_RN_ENV

info "writing react-native-bare/.env"
cat > "$ROOT_DIR/react-native-bare/.env" <<EOF_RN_BARE_ENV
# Generated by scripts/sync-secrets.sh. DO NOT COMMIT.
BACKEND_URL=$BACKEND_URL
REDIRECT_URI=$REDIRECT_URI
CLIENT_ID=$CLIENT_ID
...
# -----------------------------------------------------------------------------
# Android signing: write key.properties for all Android/Flutter/RN samples (one key)
# -----------------------------------------------------------------------------
if [[ "$TARGET_ANDROID" == "true" ]]; then
  ...
  for android_dir in android flutter/android react-native/android react-native-bare/android; do
    mkdir -p "$ROOT_DIR/$android_dir"
    cat > "$ROOT_DIR/$android_dir/key.properties" <<EOF_ANDROID_KEY
# Generated by scripts/sync-secrets.sh. DO NOT COMMIT.
...
patch_ios_pbxproj_ids() {
  ...
  IOS_BUNDLE_ID="$bundle_id" IOS_TEAM_ID="$team_id" KEEP_RUNNER_TESTS="$keep_runner_tests" perl -0777 -i -pe '
    ...
    if (defined($team) && length($team) > 0) {
      s{DEVELOPMENT_TEAM = [^;]+;}{DEVELOPMENT_TEAM = $team;}g;
      if ($_ !~ /DEVELOPMENT_TEAM = /) {
...
if [[ "$PATCH_TRACKED_DEMO_FILES" == "true" ]]; then
  ...
  if [[ "$TARGET_IOS" == "true" ]]; then
    ...
    info "  and the iOS sample's shared xcscheme (receives your clientId, backendUrl, redirectUri)"
  fi
  ...
  if [[ "$TARGET_ANDROID" == "true" ]]; then
    ...
    RN_BARE_SAMPLE_GRADLE="$ROOT_DIR/react-native-bare/android/app/build.gradle"

    for gradle_file in "$ANDROID_SAMPLE_GRADLE" "$FLUTTER_SAMPLE_GRADLE" "$RN_SAMPLE_GRADLE" "$RN_BARE_SAMPLE_GRADLE"; do
      if [[ -f "$gradle_file" ]]; then
        ...
        case "$gradle_file" in
          ...
          "$RN_SAMPLE_GRADLE")
            ...
            ;;
          "$RN_BARE_SAMPLE_GRADLE")
            patch_android_source_tree "$ROOT_DIR/react-native-bare/android/app/src" "$current_pkg" "$DEMO_ANDROID_PACKAGE_NAME"
            ;;
        esac
      fi
    done
  ...
  if [[ "$TARGET_IOS" == "true" ]]; then
    ...
    patch_ios_pbxproj_ids "$ROOT_DIR/flutter/ios/Runner.xcodeproj/project.pbxproj" "$DEMO_IOS_BUNDLE_ID" "${DEMO_IOS_TEAM_ID:-}" "false"
    patch_ios_pbxproj_ids "$ROOT_DIR/react-native/ios/KRDPASSDemo.xcodeproj/project.pbxproj" "$DEMO_IOS_BUNDLE_ID" "${DEMO_IOS_TEAM_ID:-}" "false"
    patch_ios_pbxproj_ids "$ROOT_DIR/react-native-bare/ios/KrdpassAuthBare.xcodeproj/project.pbxproj" "$DEMO_IOS_BUNDLE_ID" "${DEMO_IOS_TEAM_ID:-}" "false"
    ...
    patch_entitlements "$ROOT_DIR/react-native/ios/KRDPASSDemo/KRDPASSDemo.entitlements"
    patch_entitlements "$ROOT_DIR/react-native-bare/ios/KrdpassAuthBare/KrdpassAuthBare.entitlements"
