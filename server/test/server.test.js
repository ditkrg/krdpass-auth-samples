...
const { UpstreamError } = await import('../cas.js');
...
// Local JWT minting for tokens the server must refuse: alg none, an HS256
// forgery keyed with the RSA public key, a wrong kid, a broken signature.
// The production verifier never sees this code.
      ...
      if (url.endsWith('/connect/token')) {
        ...
        return {
          data: {
            ...
            ...(includeTokenScope ? { scope: 'openid profile' } : {}),
...
const createTransaction = (baseUrl, transactionState = state) => postJson(baseUrl, '/oauth/par', {
  codeChallenge,
...
// POST with no body at all, so req.body stays undefined and every route has to
// answer without destructuring it.
...
test('uses only stored transaction values and atomically consumes state', async () => {
  ...
  try {
    ...
    assert.equal(replay.body.error_description, 'Invalid or expired state parameter');
...
test('rejects a concurrent duplicate PAR reservation before the first PAR completes', async () => {
  ...
  try {
    ...
    assert.equal(duplicate.body.error, 'invalid_request');
    assert.equal(duplicate.body.error_description, 'An authorization transaction already exists for this state');
...
test('rejects a mismatched PKCE verifier before calling CAS', async () => {
  ...
  try {
    ...
    assert.equal(response.body.error_description, 'Invalid PKCE code verifier');
...
test('rejects fields outside the BFF token request contract without consuming state', async () => {
  ...
  try {
    ...
    assert.equal(response.body.error_description, 'Only code, codeVerifier, and state are allowed');
...
test('fetches the CAS JWKS once and reuses the imported key across exchanges', async () => {
  const fixture = await startTestServer();
  try {
    const secondState = 'state-transaction-987654321';
    await createTransaction(fixture.baseUrl);
    assert.equal((await exchangeToken(fixture.baseUrl)).status, 200);
    await createTransaction(fixture.baseUrl, secondState);
    assert.equal((await exchangeToken(fixture.baseUrl, { transactionState: secondState })).status, 200);

    const jwksFetches = fixture.calls.filter((call) => call.url === 'https://account.id.krd/keys');
...
test('constrains a refresh to the scope CAS granted at exchange', async () => {
  const fixture = await startTestServer();
  try {
    await createTransaction(fixture.baseUrl);
    assert.equal((await exchangeToken(fixture.baseUrl)).status, 200);

    const refresh = await postJson(fixture.baseUrl, '/oauth/token/refresh', {
      refreshToken: refreshTokenValue,
      environment: 'production',
      scope: 'openid profile citizen_identity offline_access',
    });
    assert.equal(refresh.status, 200);

    const refreshCall = fixture.calls.filter((call) => call.url.endsWith('/connect/token')).at(-1);
    assert.equal(refreshCall.body.get('grant_type'), 'refresh_token');
    assert.equal(refreshCall.body.get('scope'), 'openid profile');
  } finally {
    await fixture.close();
  }
});

// RFC 6749 section 6: an omitted scope asks for the grant as issued. Echoing the
// remembered grant back as an explicit request would be this server inventing one.
test('omits scope entirely when the refresh request carries none', async () => {
  const fixture = await startTestServer();
  try {
    await createTransaction(fixture.baseUrl);
    assert.equal((await exchangeToken(fixture.baseUrl)).status, 200);
    ...
    const refreshCall = fixture.calls.filter((call) => call.url.endsWith('/connect/token')).at(-1);
    assert.equal(refreshCall.body.has('scope'), false);
  } finally {
    await fixture.close();
  }
});

// A request with nothing in common with the grant narrows to nothing, and that is
// what CAS must see. Falling back to the grant here would widen the request instead.
test('sends an empty scope when the refresh request is disjoint from the grant', async () => {
  const fixture = await startTestServer();
  try {
    await createTransaction(fixture.baseUrl);
    assert.equal((await exchangeToken(fixture.baseUrl)).status, 200);
    ...
    const refreshCall = fixture.calls.filter((call) => call.url.endsWith('/connect/token')).at(-1);
    assert.equal(refreshCall.body.get('scope'), '');
  } finally {
    await fixture.close();
  }
});

// A scope-less refresh against a CAS response that carries no scope either teaches
// this server nothing new, so the grant it already knows has to survive the rotation.
test('keeps the known grant across a refresh that sends and receives no scope', async () => {
  const fixture = await startTestServer({ includeTokenScope: false });
  try {
    await createTransaction(fixture.baseUrl);
    assert.equal((await exchangeToken(fixture.baseUrl)).status, 200);

    const first = await postJson(fixture.baseUrl, '/oauth/token/refresh', {
      refreshToken: refreshTokenValue,
      environment: 'production',
    });
    assert.equal(first.status, 200);

    const second = await postJson(fixture.baseUrl, '/oauth/token/refresh', {
      refreshToken: refreshTokenValue,
      environment: 'production',
...
// No grant remembered (a restart, or a refresh token minted elsewhere): the request
// goes through untouched and CAS enforces the grant itself.
test('passes a refresh scope through when the grant is unknown to this process', async () => {
  const fixture = await startTestServer();
  try {
    const refresh = await postJson(fixture.baseUrl, '/oauth/token/refresh', {
    ...
    const refreshCall = fixture.calls.filter((call) => call.url.endsWith('/connect/token')).at(-1);
    assert.equal(refreshCall.body.get('scope'), 'openid profile citizen_identity');
  } finally {
    await fixture.close();
  }
});

test('rejects a refreshed ID token whose signature does not hold', async () => {
  const fixture = await startTestServer({ tamperIdTokenSignature: true });
  try {
    const response = await postJson(fixture.baseUrl, '/oauth/token/refresh', {
      refreshToken: refreshTokenValue,
      environment: 'production',
    });
    assertSanitizedUpstreamFailure(response);
...
test('rejects a redirect URI whose host is not in the allowlist', async () => {
  ...
  try {
    ...
    assert.equal(response.status, 400);
    assert.equal(response.body.error, 'invalid_request');
    assert.equal(response.body.error_description, 'redirectUri host is not in server allowlist');
    assert.equal(fixture.calls.length, 0);
  } finally {
    await fixture.close();
  }
...
test('rate limits /oauth to 30 requests per minute per client', async () => {
  ...
  try {
    ...
    // The limiter covers /oauth only, so /health stays answerable.
...
test('maps upstream CAS status codes instead of mirroring them', async () => {
  // A real UpstreamError, not a duck-typed one: sendUpstreamError reads the CAS status only
  // off the one type that defines it, so any other error maps to 502 regardless of its fields.
  const upstream = (status) => () => new UpstreamError('CAS failed', { upstreamStatus: status });
  const cases = [
